...
open class NetIdService: NSObject {
    ...
    /**
     ...
     - Parameter authFlow: Type of flow to use, can be either ``NetIdAuthFlow.Permission``, ``NetIdAuthFlow.Login`` or ``NetIdAuthFlow.LoginPermission``
     - Parameter forceApp2App: If set to true, will yield an ``NetIdError`` if the are no ID apps installed. Otherwise, will use app2web flow automatically. Defaults to ``false``.
     - Returns: An array containing all buttuns that would be displayed in the view returned by ``getAuthorizationView``
     */
/*    public func getAuthorizationButtons(currentViewController: UIViewController, authFlow: NetIdAuthFlow, forceApp2App: Bool = false) -> [AnyView] {
        let netIdApps = AuthorizationWayUtil.checkNetIdAuth()
        // If there are no ID apps installed, but forceApp2App is true, return with an error.
        if netIdApps.isEmpty && forceApp2App {
            self.didFinishAuthenticationWithError(
        ...
        case .Permission:
            let config = netIdConfig?.permissionLayerConfig
            return getPermissionButtons(delegate: self, presentingViewController: currentViewController, appIdentifiers: netIdApps, logoId: (config?.logoId) ?? "", headlineText: (config?.headlineText) ?? "", legalText: (config?.legalText) ?? "", continueText: (config?.continueText) ?? "")
        case .Login, .LoginPermission:
            let config = netIdConfig?.loginLayerConfig
            return getLoginButtons(delegate: self, presentingViewController: currentViewController, appIdentifiers: netIdApps, headlineText: (config?.headlineText) ?? "", loginText: (config?.loginText) ?? "", continueText: (config?.continueText) ?? "")
        }
    }
    
    private func getPermissionButtons(delegate: AuthorizationViewDelegate, presentingViewController: UIViewController, appIdentifiers: [AppIdentifier], logoId: String, headlineText: String, legalText: String, continueText: String) -> [AnyView] {
        var buttons = [AnyView]()
        var index = 0
        for result in appIdentifiers {
            ...
            buttons.append(AnyView(stack))
            index += 1
        }

        let button = Button {
            var destinationScheme: String?
            if appIdentifiers.count > self.selectedAppIndex {
                let selectedAppIdentifier = appIdentifiers[self.selectedAppIndex]
                destinationScheme = selectedAppIdentifier.iOS.universalLink
            }
            self.didTapContinue(destinationScheme: destinationScheme, presentingViewController: presentingViewController, authFlow: NetIdAuthFlow.Permission)
        } label: {
            ...
            Text(continueText.isEmpty ? LocalizableUtil.netIdLocalizable("authorization_view_agree_and_continue_with_net_id") : continueText)
                ...
                .background(Color("netIdOtherOptionsColor", bundle: Bundle(for: NetIdService.self)))
                .overlay(RoundedRectangle(cornerRadius: 5).stroke(Color("closeButtonGrayColor", bundle: Bundle(for: NetIdService.self))))
        buttons.append(AnyView(button))
        
        return buttons
    }

    private func getLoginButtons(delegate: AuthorizationViewDelegate, presentingViewController: UIViewController, appIdentifiers: [AppIdentifier], headlineText: String, loginText: String, continueText: String) -> [AnyView] {
        var buttons = [AnyView]()
        for result in appIdentifiers {
            let button = Button {
                self.didTapContinue(destinationScheme: result.iOS.universalLink, presentingViewController: presentingViewController, authFlow: .Login)
        ...
        .padding(12)
        .background(Color("netIdContinueColor", bundle: Bundle(for: NetIdService.self)))
        .cornerRadius(5)
        .padding(.horizontal, 20)
        buttons.append(AnyView(button))
